...
async function safeDeleteStorage(path) {
    if (!path)
        return;
    try {
...
async function deleteMessageMedia(chatId, messageId, storagePath) {
    if (storagePath) {
...
export const cleanupExpiredMessages = onSchedule({ schedule: "every 60 minutes", timeoutSeconds: 540 }, async () => {
    ...
    for (const chat of chatsSnap.docs) {
        ...
        while (true) {
            ...
            if (msgs.size < BATCH_LIMIT)
                break;
            lastDoc = msgs.docs[msgs.docs.length - 1];
            query = chat.ref
                .collection("messages")
                .where("expiresAt", "<=", cutoff)
                .startAfter(lastDoc)
...
export const cleanupExpiredStatuses = onSchedule({ schedule: "every 30 minutes", timeoutSeconds: 540 }, async () => {
...
export const onMessageCreated = onDocumentCreated({
    ...
    if (!receiverId)
        return;
    const preview = data.type === "image" ? "📷 Photo" : data.type === "video" ? "🎥 Video" : (data.text ?? "").slice(0, 80);
    const notifRef = db
        .collection("users")
        .doc(receiverId)
        .collection("notifications")
...
export const deleteAccount = onCall({ timeoutSeconds: 300 }, async (req) => {
    ...
    fq.docs.forEach((d) => {
        d.data().userIds.forEach((id) => id !== uid && peerIds.add(id));
    });
